...
const pageHeader=document.querySelector('.page-header__nav-list');const headerToggle=document.querySelector('.page-header__menu-button');headerToggle.addEventListener('click',()=>{if(pageHeader.classList.contains('page-header__nav-list--hide')){pageHeader.classList.remove('page-header__nav-list--hide');}else{pageHeader.classList.add('page-header__nav-list--hide');}});const portfolioSwiper=new Swiper(".portfolio__swiper-container",{slidesPerView:'auto',spaceBetween:10,loop:true,slidesPerGroupAuto:true,wrapperClass:"portfolio__list",slideClass:"portfolio__item",navigation:{nextEl:".portfolio__list-next",prevEl:".portfolio__list-prev",},})
const servicesMasterBlocks=document.querySelectorAll('.services__master-change')
servicesMasterBlocks.forEach((block,index)=>{const currentMasterItem=block.querySelector('.services__current-master')
...
currentMasterItem.addEventListener('click',(evt)=>{block.classList.toggle('services__master-change--border')
...
servicesMasterTypes.forEach((type)=>{type.addEventListener('click',(evt)=>{currentMasterItem.textContent=type.textContent
...
servicesOfferButton.addEventListener('click',()=>{servicesOfferButton.classList.add('services__offer-button--hide')
...
serviceItemPrices.forEach((price)=>{const currentPrice=price.querySelector('.services__service-current-price')
...
document.querySelectorAll('.page-header__nav-item').forEach((link)=>{link.addEventListener('click',(evt)=>{evt.preventDefault();document.querySelectorAll('.js-section').forEach((section,i)=>{if(link.dataset.id===section.getAttribute('id')){window.scrollTo({behavior:'smooth',top:section.offsetTop,});}})})})
...
if(i>=0&&i<aboutServicesBlocks.length){aboutServicesBlock.classList.add('about__description-services--show')
aboutServicesPrevBlock.classList.remove('about__description-services--show')}else{aboutServicesPrevBlock.classList.remove('about__description-services--show')
